...
const App = () => {
	...
	const resetBasketSettings = useCallback(() => {
		setFaster(true);
		setTime('');
		setSelfService(false);
	}, [setTime, setFaster, setSelfService])
			...
			<Switch>
				...
				<Route path="/order/:areaId/:itemId" exact>
					<Order
						...
						order={order}
						resetBasketSettings={resetBasketSettings}
						...
						return (
							<Place
								...
								onIncrementPosition={({ id, itemId, areaId }) => {
									...
									const productId = `${id}_${itemId}`

									if (productId in updatedOrder) {
										updatedOrder[productId].count++;
									} else {
										updatedOrder[productId] = {
											item: foodsMap[productId],
											count: 1,
										};
									}
									...
									if (Object.keys(nextOrderStatuses).length === 0) {
										FOOD_AREAS.forEach(area => {
											area.items.forEach(item => {
												item.foods.forEach(food => {
													const productId = `${food.id}_${item.id}`
								...
								onDecrementPosition={({ id, itemId, areaId }) => {
									...
									if (productId in updatedOrder) {
										if (updatedOrder[productId].count === 1) {
